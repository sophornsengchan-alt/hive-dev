...
@ Purpose:        US-0011623 - Bulk Upload Unsub Deals for AU
----------------------------------------------------------------------------------------------------------------------------------
@ Change history: 15.08.2022 / Vadhanak Voun / Replicated from NA version for US-0011623
@               : 11.10.2022/vadhanak voun/ US-0012765 - Improve upload tool in SEP to return number properly instead of string
...
import { ShowToastEvent } from 'lightning/platformShowToastEvent';//SB 17.03.2023 US-0013185
...
import bulkUploadDealTemplateAU from '@salesforce/resourceUrl/SEP_Bulk_Upload_Deal_Template_AU';
...
import AuDwAgmtMdtName from '@salesforce/label/c.AU_Deal_Window_Agreement_Terms_MDT_Name'; //SP 15-3-2023 US-0012238
import btn_back from '@salesforce/label/c.SEP_BACK_BUTTON';//SB 17.03.2023 US-0013185
import thank_You_message from '@salesforce/label/c.Thank_You_message';//SB 17.03.2023 US-0013185
import dealBulkUpload_HelpMessage from '@salesforce/label/c.dealBulkUpload_HelpMessage';//SB 24.03.2023 US-0013185

export default class DealBulkUploadAU extends NavigationMixin(LightningElement) {
    @api tabName = "";
    @api recordId;
    @api startDate;
    @api endDate;
    ...
    @track csvHeader1  = 'ebay Item Id,Deal Price,RRP,Quantity,Maximum Purchases';
   ...
   // @track csvHeaderGerman2 = '"eBay-Artikelnr","Artikelbezeichnung des eBay-Angebots","EAN","Artikel Zustand","Artikelstückzahl für WOW! Angebot","Angebotspreis des Verkäufers","Format des WOW! Angebots","Artikel frühestens verfügbar für WOW! Angebot ab","Preisvergleichsportal 1 (Amazon für DE)","Preisvergleichsportal 2 (Idealo für DE)","eBay-Hauptkategorie","Kommentare des Verkäufers"';
    //SB 27.03.2023 US-0013185 remove width from columns to make it fit the screen
   @track defaulCols = [
        ...
        { label: 'Deal Price', fieldName: 'EBH_DealPrice__c', type: 'currency', typeAttributes: { currencyCode: 'AUD',currencyDisplayAs:'code', step: '0.001'}},
        //{ label: 'List Price', fieldName: 'EBH_RRPWASPrice__c', type: 'currency', initialWidth: 160, typeAttributes: { currencyCode: 'USD', step: '0.001'}},
        { label: 'RRP/WAS Price', fieldName: 'EBH_RRPWASPrice__c', type: 'currency', typeAttributes: { currencyCode: 'AUD',currencyDisplayAs:'code', step: '0.001'}},
        { label: 'Quantity', fieldName: 'EBH_Quantity__c', type: 'number'},
        { label: 'Maximum Purchases', fieldName: 'EBH_MaximumPurchases__c', type: 'number'}
    ...
    @track requiredDealFields = ["EBH_eBayItemID__c", "EBH_DealPrice__c", "EBH_RRPWASPrice__c", "EBH_Quantity__c", "EBH_MaximumPurchases__c"];
    @track isUnableUpload = false; //BR-24-10-22-US-0012600
    @track currentStep = 1;//SB 17.03.2023 US-0013185

    @track showTC = false; // 15.03.2023 / Sophal Noch / US-0012238
    @track isTCAgreed = false; // 15.03.2023 / Sophal Noch / US-0012238
    @track mapArgs = {}; // 16.03.2023 / Sophal Noch / US-0012238
    ...
    label = {LWCBulkUploadCSVError1, LWCBulkUploadCSVError2, LWCBulkUploadCSVError3,LWCBulkUploadCSVError4,LWCBulkUploadCSVError5,
        ...
        DRC_Inline_info, PleaseSelectTheEBayAccount, AuDwAgmtMdtName,btn_back,thank_You_message,dealBulkUpload_HelpMessage
    };
    ...
    doLoadCMT(){
        ...
        .then(result => {
            ...
            if(result["status"] == "success"){
                ...
                // console.log('result.availableDeal : >>> : ' + result.availableDeal);
    ...
    handleFilesChange(event) {
        ...
        if(event.target.files.length > 0) {
            ...
            this.isTCAgreed = false; // 15.03.2023 / Sophal Noch / US-0012238
    ...
    onAgreementTermSEPInit(event){
        ...
        if(this.label.AuDwAgmtMdtName == event.detail["agmtMdtName"]){
    ...
    onAgreementTermSEPAccepted(event){
        ...
        if(this.label.AuDwAgmtMdtName == event.detail["agmtMdtName"]){
            ...
            if(accepted){
                // when user agree to term and condition, procced to submit deals
                this.isTCAgreed = true; // 15.03.2023 / Sophal Noch / US-0012238
            ...
            if(this.validateHeader(csvHeader,this.csvHeader1)){
                ...
                for(var i = 1; i < allTextLines.length; i++) {
                    ...
                    for(var x= 0; x < allCols.length; x++) {
                        ...
                        }else if(colName == "EBH_RRPWASPrice__c") 
                        {

                        }else if(colName == "EBH_MaximumPurchases__c") 
                        {
                            //Error message when: Value has to be equal or lower than Quantity field, as this field restricts individuals from purchasing more than a set number of items per listing.
                            //Error message copy: Error [Maximum Purchases] Incorrect Format
                            if(this.isNumeric(val)) {
                                if(parseInt(val) >deal.EBH_Quantity__c){     //NK:11/10/2022/US-0012765
    ...
    handleTCAndClickSubmit(){ // 15.03.2023 / Sophal Noch / US-0012238 : show agreement term before allow deals to be submitted
        if(!this.isTCAgreed){
            this.showTC = true;
            this.showLoadingSpinner = true;
        }else{
    ...
    handleClickSubmit() {
        ...
        this.message = "";
        this.currentStep++;// SB 17.03.2023 US-0013185
    ...
    doDownloadCSVTemplate(){
        ...
        downloadElement.href = bulkUploadDealTemplateAU; //MN-14122021-US-0010945- Download the NA Template via Static Resource
        downloadElement.target = '_self';
        // CSV File Name
        downloadElement.download = "AU_unsub_Deals_Upload - Seller.csv"; 
    ...
    handleAccountChange(event) {
        ...
        this.mapArgs['accountId'] = this.accountId; // 16.03.2023 / Sophal Noch / US-0012238
        this.mapArgs['drcId'] = this.recordId; // 16.03.2023 / Sophal Noch / US-0012238
        this.doLoadCMT();
    }
