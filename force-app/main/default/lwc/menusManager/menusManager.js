...
import fetchSPMainDomains from '@salesforce/apex/menusManagerController.getSPMainDomains';
import fetchRevRollUp from '@salesforce/apex/menusManagerController.getRevRollUp'; //MN-24092025-US-0033439
...
const MENU_ITEM_COLUMNS_DEFINITION = [
    ...
    { //MN-13062023-US-0013379 
        type: 'text',
        fieldName: 'spMainDomain',
        label: 'SP Main Domain',
        initialWidth: 250,
    },
    { //MN-24092025-US-0033439
        type: 'text',
        fieldName: 'revRollUp',
        label: 'Rev Roll Up',
        initialWidth: 250,
...
const cleanMenuItemFields = ['position','openInNewWindow','level','language','label','_children','isPublic','iconPosition','icon','href','spMainDomain', 'revRollUp']; //MN-13062023-US-0013379 //MN-24092025-US-0033439
...
export default class MenusManager extends LightningElement {
    ...
    @track spMainDomainList; //MN-13062023-US-0013379
    @track spMainDomainOptions; //MN-13062023-US-0013379
    @track spMainDomainListResult; //MN-13062023-US-0013379
    @track revRollUpList; //MN-24092025-US-0033439
    @track revRollUpOptions; //MN-24092025-US-0033439
    @track revRollUpListResult; //MN-24092025-US-0033439
    @track menuItemList;
    @track menuItemMap;
    @track menuItemListResult;
    @track menuId = '';
    @track languageFilter = '';
    @track spMainDomainFilter = ''; //MN-13062023-US-0013379
    @track revRollUpFilter = ''; //MN-24092025-US-0033439
    @track menuName;
    ...
    //wire functions
    wireFetchMenu;
    @wire(fetchMenu,{menuId: '$menuId', language: '$languageFilter', spMainDomain: '$spMainDomainFilter', revRollUp: '$revRollUpFilter'})
    ...
    wireFetchSPMainDomains;
    @wire(fetchSPMainDomains)
    fetchSPMainDomainsImperativeWiring(result) 
    {
        if (result.data) {
            this.spMainDomainListResult = result;
            this.spMainDomainList = JSON.parse(result.data);
            this.spMainDomainOptions = new Array();
            for(let i=0; i<this.spMainDomainList.length;i++)
            {
                let tmpspMainDomainOption = {};
                tmpspMainDomainOption.label = this.spMainDomainList[i];
                tmpspMainDomainOption.value = this.spMainDomainList[i];
                tmpspMainDomainOption.value = (tmpspMainDomainOption.value === 'none') ? '' : tmpspMainDomainOption.value ;
                this.spMainDomainOptions.push(tmpspMainDomainOption);
            }
            this.error = undefined;
        } else if (result.error) {
            this.spMainDomainListResult = result;
            this.error = result.error;
            this.spMainDomainList = undefined;
        }
    }

    wireFetchRevRollUp;
    @wire(fetchRevRollUp)
    fetchRevRollUpImperativeWiring(result) 
    {
        if (result.data) {
            this.revRollUpListResult = result;
            this.revRollUpList = JSON.parse(result.data);
            this.revRollUpOptions = new Array();
            for(let i=0; i<this.revRollUpList.length;i++)
            {
                let tmpRevRollUpOption = {};
                tmpRevRollUpOption.label = this.revRollUpList[i];
                tmpRevRollUpOption.value = this.revRollUpList[i];
                tmpRevRollUpOption.value = (tmpRevRollUpOption.value === 'none') ? '' : tmpRevRollUpOption.value ;
                this.revRollUpOptions.push(tmpRevRollUpOption);
            }
            this.error = undefined;
        } else if (result.error) {
            this.revRollUpListResult = result;
            this.error = result.error;
            this.revRollUpList = undefined;
        }
    }
    ...
    //MN-13062023-US-0013379
    handleSPMainDomainChange(e) 
    {
        this.spMainDomainFilter = e.detail.value;
        this.spMainDomainFilter = (this.spMainDomainFilter === 'none') ? '' : this.spMainDomainFilter ;
        ...
        try {
            this.template.querySelector('[data-name="spMainDomains"]').blur();
        } catch(e){}

    }

    //MN-24092025-US-0033439
    handleRevRollUpChange(e) 
    {
        this.revRollUpFilter = e.detail.value;
        this.revRollUpFilter = (this.revRollUpFilter === 'none') ? '' : this.revRollUpFilter ;
        ...
        try {
            this.template.querySelector('[data-name="revRollUps"]').blur();
        } catch(e){}
